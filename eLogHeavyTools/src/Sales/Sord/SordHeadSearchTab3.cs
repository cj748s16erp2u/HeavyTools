...
{
    public class SordHeadSearchTab3 : SordHeadSearchTab, eProjectWeb.Framework.Xml.IXmlObjectName
    {
        #region IXmlObjectName

        protected UploadButton sordHeadImportButton;
        protected DialogBox dlgSimpleMessage3;

        protected static Type baseType = typeof(SordHeadSearchTab);
        ...
        protected override void CreateControls()
        {
            ...
            this.sordHeadImportButton = this.AddCmd(new UploadButton("sordheadimport", 950));
            this.SetButtonAction(this.sordHeadImportButton.ID, this.SordHeadImportButtonClicked);

            this.dlgSimpleMessage3 = new DialogBox(DialogBoxType.Ok);
            this.RegisterDialog(this.dlgSimpleMessage3);
        }

        private void SordHeadImportButtonClicked(PageUpdateArgs args)
        {
            var uploadInfo = this.sordHeadImportButton.GetUploadData(args);

            var sordHeadBL = (SordHeadBL3)SordHeadBL.New();
            var processResult = sordHeadBL.SordHeadImport(uploadInfo);
            ...
            if (!string.IsNullOrWhiteSpace(fileName))
            {
                ...
                var sordHeadImportTransl = "$sordheadimport".eLogTransl();
                var realFileName = $"{sordHeadImportTransl}_{DateTime.Now:yyyyMMddHhmmss}";
