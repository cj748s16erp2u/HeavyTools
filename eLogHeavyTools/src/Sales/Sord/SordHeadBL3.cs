...
using eLog.HeavyTools.Sales.Sord.Import;
...
namespace eLog.HeavyTools.Sales.Sord
{
    public class SordHeadBL3 : SordHeadBL
    {
        ...
        public ImportResult SordHeadImport(UploadData uploadInfo)
        {
            utils = new ImportBase.ImportUtils();
            string error = null;

            var importDescrFileName = CustomSettings.GetString("SordHeadImportDescrFileName");
            if (string.IsNullOrWhiteSpace(importDescrFileName))
            {
                error = "$err_partnerimport_missingsettings".eLogTransl("SordHeadImportDescrFileName");
            }

            var descrDirs = CustomSettings.GetString("ImportDescrFolders");
            if (string.IsNullOrWhiteSpace(descrDirs))
            {
                error = "$err_sordheadimport_missingsettings".eLogTransl("ImportDescrFolders");
            }
            else
            {
        ...
        private ImportResult ImportPartnerXlsxFiles(IEnumerable<eProjectWeb.Framework.UI.Controls.UploadFileInfo> uploadFiles, string importDescrFileName, string descrDirs)
        {
            ...
            var importResultFolder = Path.Combine(Globals.WritableRoot, "SordHeadImport");
            ...
            using (new eProjectWeb.Framework.Lang.NS(typeof(SordHeadBL3).Namespace))
            {
        ...
        {
            var importService = new SordHeadImportService();
            ...
            try
            {
                Log.Info($"{executeId} - {Session.UserID} executing sordhead import: {fileName} ({storedFileName})");
