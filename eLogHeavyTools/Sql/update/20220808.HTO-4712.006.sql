-- new fields: olc_action.curid
/*
exec sp_insertcolumn
  @tablename     = 'olc_action',
  @columndef     = 'curid varchar(12) ',
  @refcolumnname = 'singlecouponnumber',
  @after         = 0,
  @customsql     = 'alter table olc_action add constraint fk_olc_action_curid foreign key (curid) references ols_currency (curid)',
  @showonly      = 1
*/
...
go
if not exists(select 0 from sys.columns where object_id = object_id('olc_action') and name = 'curid')    alter table olc_action add curid varchar(12)
go
...
go
alter table olc_action add constraint fk_olc_action_curid foreign key (curid) references ols_currency (curid)
go
