-- new fields: olc_action.note
/*
exec sp_insertcolumn
  @tablename     = 'olc_action',
  @columndef     = 'note varchar(max)',
...
go
if not exists(select 0 from sys.columns where object_id = object_id('olc_action') and name = 'note')    alter table olc_action add note varchar(max)
go
