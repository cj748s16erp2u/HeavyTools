-- new fields: olc_sordhead.bustypeid
/*
exec sp_insertcolumn
  @tablename     = 'olc_sordhead',
  @columndef     = 'bustypeid varchar(12) null ',
  @refcolumnname = 'addusrid',
  @after         = 0,
  @customsql     = 'alter table olc_sordhead add constraint fk_olc_sordhead_bustypeid foreign key (bustypeid) references ols_bustype (bustypeid)',
  @showonly      = 1
*/
...
go
if not exists(select 0 from sys.columns where object_id = object_id('olc_sordhead') and name = 'bustypeid')    alter table olc_sordhead add bustypeid varchar(12) null
go
