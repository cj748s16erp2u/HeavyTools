-- new fields: olc_action.isext
/*
exec sp_insertcolumn
  @tablename     = 'olc_action',
  @columndef     = 'isext int null',
  @refcolumnname = 'name',
  @after         = 0,
  @customsql     = '',
  @showonly      = 1
...
go
if not exists(select 0 from sys.columns where object_id = object_id('olc_action') and name = 'isext')    alter table olc_action add isext int null
go
...
update olc_action set isext=0
update olc_action set isactive=1

go
alter table olc_action alter column isext int not null 
alter table olc_action alter column isactive int not null

go
 

EXEC sp_rename 'olc_action.isext', 'isextcondition', 'COLUMN'; 