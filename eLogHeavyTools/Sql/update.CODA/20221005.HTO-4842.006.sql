	...
	ELSE BEGIN
		...
		IF @ok = 1 BEGIN
			...
			SELECT 
				...
				ROUND(transaction_amount, ISNULL(CD.decnum,0)), -- valuedoc
				transaction_amount / ABS(transaction_amount), -- amountsign
				currency, -- origcur
				--transaction_amount, -- origvalue
				ROUND(transaction_amount, ISNULL(CD.decnum,0)), -- origvalue
				...
				terminal_id, -- extref6
				ROUND(commission_amount, ISNULL(CD.decnum,0)) -- valueacc, jutalek
			FROM cif_ebank_otp_card_lines L WITH (NOLOCK)
