if exists(select 0 from sysobjects where name = 'cif_ebank_fio_load' and xtype = 'P')
  drop procedure dbo.cif_ebank_fio_load
go

CREATE PROCEDURE cif_ebank_fio_load
...
BEGIN
	...
	SELECT TOP 1 @filename=filname FROM cif_ebank_fio_header WITH (NOLOCK) WHERE interfaceid=@interfaceid AND username=@userid 

	UPDATE cif_ebank_fio_075
	...
	IF NOT EXISTS (
		SELECT 1
		FROM cif_ebank_fio_header WITH (NOLOCK)
		WHERE interfaceid=@interfaceid AND username=@userid 
	) 
	...
	ELSE BEGIN
			...
			WHILE @@FETCH_STATUS = 0  BEGIN
				...
				ELSE BEGIN
					...
					FROM cif_ebank_fio_075 l75 WITH (NOLOCK) 
					LEFT OUTER JOIN cif_ebank_fio_075 NXT WITH (NOLOCK) ON NXT.interfaceid=@interfaceid AND NXT.username=@userid AND NXT.lineseqno > l75.lineseqno
					...
					FROM cif_ebank_fio_075 l75 WITH (NOLOCK) 
						JOIN cif_ebank_fio_header h WITH (NOLOCK) ON h.interfaceid = l75.interfaceid AND h.username = l75.username AND l75.lineseqno BETWEEN @this AND @next
					...
					ELSE BEGIN
						INSERT INTO cif_errors (interfaceid, linkcode, filename, step, messagecode, extramessage, usr, createtime, critical)
							...
							FROM cif_ebank_fio_075 L WITH (NOLOCK)
			...
			IF @ok = 1 BEGIN
				...
				WHILE @@FETCH_STATUS = 0 BEGIN
					...
					SELECT 
						...
						case when LTRIM(RTRIM(ISNULL(l78.transaction_info_p1, N''))) = N''
						...
						@opening, -- openbalacc
						@closing -- closebalacc
					FROM cif_ebank_fio_075 l75 (nolock)
						left outer join cif_ebank_fio_076 l76 (nolock) on l76.interfaceid=l75.interfaceid AND l76.username=l75.username AND l76.lineseqno=l75.lineseqno+1
						left outer join cif_ebank_fio_078 l78 (nolock) on l78.interfaceid=l76.interfaceid AND l78.username=l76.username AND l78.lineseqno BETWEEN l76.lineseqno+1 AND l76.lineseqno+2
						left outer join cif_ebank_fio_079 l79 (nolock) on l79.interfaceid=l78.interfaceid AND l79.username=l78.username AND l79.lineseqno=l78.lineseqno+1
					WHERE 
